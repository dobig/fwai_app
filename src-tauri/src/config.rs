...
///   且不一定等于用户目录，可能导致 `.fwai_app/fwai_app.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取应用配置目录路径 (~/.fwai_app)
///
/// fwai_app 只读写自己的目录，绝不碰 `~/.cc-switch`。
///
/// 早期版本会在首次启动时把 `~/.cc-switch/cc-switch.db` 拷过来，本意是给我们
/// 自己 v1.1.0 及更早版本（那时配置目录还叫 `.cc-switch`）的用户做改名迁移。
/// 但那个目录名和上游 cc-switch 完全相同，代码无从分辨库是谁写的：装过上游
/// 的机器会把上游的库拷进来，而上游的 schema 版本比这里高，启动时撞上
/// "数据库版本过新" 直接打不开（见 database/schema.rs 的版本检查）。
///
/// 两个应用必须各自独立，所以迁移整段删除。代价是从 v1.1.0 直升上来的用户
/// 需要重新添加供应商——他们的数据仍原封不动躺在 `~/.cc-switch`，只是不再被
/// 自动搬运。
