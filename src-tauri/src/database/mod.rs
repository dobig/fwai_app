...
/// 把 schema 版本高于本应用的数据库改名让路，返回是否归档过。
///
/// 只处理 "版本过新" 这一种情况：那是早期版本从上游 cc-switch 拷来的库留下的，
/// 本应用没有能力读它（迁移是单向的，没有降级路径），留在原地只会让应用每次
/// 启动都失败。版本正常、损坏、打不开的库一律不动——那些该走既有的错误弹窗，
/// 不能被这里顺手改名。
///
/// 改名而不是删除：库里是用户的供应商配置，可能是他们唯一的副本。
fn archive_incompatible_database(db_path: &Path) {
    if !db_path.exists() {
        return;
    }
    // 只读地问一句版本号。打不开就交给正常的初始化流程去报错。
    let version = match Connection::open(db_path) {
        Ok(conn) => match Database::get_user_version(&conn) {
            Ok(v) => v,
            Err(e) => {
                log::warn!("读取数据库版本失败，交由初始化流程处理: {e}");
                return;
            }
        },
        Err(e) => {
            log::warn!("打开数据库失败，交由初始化流程处理: {e}");
            return;
        }
    };
    ...
    // 带上版本号，且不覆盖已有归档：连装两次的机器不该把上一份归档冲掉。
    let mut archived = db_path.with_extension(format!("db.incompatible-v{version}"));
    let mut suffix = 1;
    while archived.exists() {
        archived = db_path.with_extension(format!("db.incompatible-v{version}.{suffix}"));
        suffix += 1;
    }
    match std::fs::rename(db_path, &archived) {
        Ok(()) => log::warn!(
            "数据库版本 {version} 高于本应用支持的 {SCHEMA_VERSION}（来自上游 cc-switch 的遗留拷贝），\
             已归档为 {} 并新建空数据库",
            archived.display()
        ),
        Err(e) => log::error!("归档不兼容数据库失败: {e}"),
    }
}
...
impl Database {
    /// 初始化数据库连接并创建表
    ///
    /// 数据库文件位于 `~/.fwai_app/fwai_app.db`
    pub fn init() -> Result<Self, AppError> {
        let db_path = get_app_config_dir().join("fwai_app.db");
        ...
        // 早期版本会把上游 cc-switch 的数据库拷进来（见 config.rs 里已删除的
        // 迁移），拷来的库 user_version 比这里高，迁移检查会直接报 "版本过新"
        // 并让应用打不开。迁移已经切断，但那些机器上的库还躺在磁盘上，光删代码
        // 不会自愈——这里把它归档掉再建新库。
