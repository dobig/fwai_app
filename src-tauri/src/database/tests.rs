...
/// 早期版本会把上游 cc-switch 的数据库拷进 `~/.fwai_app`，拷来的库 user_version
/// 比本应用高，启动时被迁移检查挡下（见 schema_migration_rejects_future_version），
/// 表现为应用打不开。迁移已经切断，但已中招机器上的库还在磁盘上，需要归档让路。
#[test]
fn archives_database_from_newer_upstream() {
    let dir = tempfile::tempdir().expect("temp dir");
    let db_path = dir.path().join("fwai_app.db");

    // 模拟一份上游 cc-switch 的库（当前上游是 16）
    let conn = Connection::open(&db_path).expect("create db");
    Database::create_tables_on_conn(&conn).expect("create tables");
    Database::set_user_version(&conn, SCHEMA_VERSION + 6).expect("set upstream version");
    drop(conn);

    archive_incompatible_database(&db_path);

    assert!(!db_path.exists(), "不兼容的库应被移走，给新库让路");
    let archived = db_path.with_extension(format!("db.incompatible-v{}", SCHEMA_VERSION + 6));
    assert!(archived.exists(), "用户数据必须归档保留，不能删除");
    // 归档的确实是原来那份
    let reopened = Connection::open(&archived).expect("open archived");
    assert_eq!(
        Database::get_user_version(&reopened).expect("archived version"),
        SCHEMA_VERSION + 6
    );
}

/// 版本正常的库绝不能被动——归档只针对 "版本过新" 这一种情况。
#[test]
fn keeps_database_at_supported_version() {
    let dir = tempfile::tempdir().expect("temp dir");
    let db_path = dir.path().join("fwai_app.db");
    ...
    assert!(db_path.exists(), "当前版本的库必须原地保留");
    assert_eq!(
...
/// 尚未迁移过的老库（user_version=0）同样不能被归档：它要走正常的迁移路径。
#[test]
fn keeps_unmigrated_database() {
    let dir = tempfile::tempdir().expect("temp dir");
    let db_path = dir.path().join("fwai_app.db");
    ...
    assert!(db_path.exists(), "user_version=0 的库应留给迁移流程处理");
}

/// 连装两次的机器不该把上一份归档冲掉。
#[test]
fn archive_does_not_clobber_existing_archive() {
    let dir = tempfile::tempdir().expect("temp dir");
    let db_path = dir.path().join("fwai_app.db");
    let future = SCHEMA_VERSION + 6;

    for _ in 0..2 {
        let conn = Connection::open(&db_path).expect("create db");
        Database::create_tables_on_conn(&conn).expect("create tables");
        Database::set_user_version(&conn, future).expect("set upstream version");
        drop(conn);
        archive_incompatible_database(&db_path);
    }

    assert!(db_path
        .with_extension(format!("db.incompatible-v{future}"))
        .exists());
    assert!(db_path
        .with_extension(format!("db.incompatible-v{future}.1"))
        .exists());
}
