...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".fwai_app").join("settings.json")
}
...
"#
    ...
    let new_config_text = new_provider
        ...
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    // 供应商配置快照应保留 provider 自己的 config 内容
    assert!(
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = fwai_app_lib::get_claude_settings_path();
    ...
    assert!(
        db_path.exists(),
        "switching provider should persist to fwai_app.db"
    );
